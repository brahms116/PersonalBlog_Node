...
export default class Root {
    ...
    public root = {
        async createReader(input: ReaderInput) {
            // console.log(input)
            try {
                const reader = Reader.createReaderFromData({
                    email: input.email,
                });
        ...
        async createPost({ input }: PostInputData) {
            try {
                const post = Post.createFromData(
                    input ? { createdAt: Date.now(), ...input } : {}
                );
        ...
        async deleteReader(input: { id: string }) {
            try {
                const reader = Reader.createReaderFromData({ ref: input.id });
